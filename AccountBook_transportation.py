...
class AccountBook_trans(QMainWindow):
    def __init__(self):
        super().__init__()
        # 设置初始数值
        self.name_selected = ''
    ...
    def export_DB_months_all(self):
        if self.sqlite_exist():
            ...
            if msgBox.exec_() == QMessageBox.AcceptRole:
                ...
                # 数据库内数据导出到Excel表格
                conn = sqlite3.connect('db_trans.sqlite')
                cursor = conn.cursor()
    ...
    def update_info(self):
        ...
        if infos:
            for info in infos:
                ...
                self.ui.yearly_lineEdit.setText(
                    str(round(self.total_selected, 2)))
        ...
        if infos:
            ...
            for info in infos:
                string = str(info[0]) + ":于" + str(info[1]) + ', 提交:' + str(
                    round(info[2], 2)) + ', 共支取:' + str(round(
                        info[3], 2)) + ', 剩余:' + str(round(info[4], 2))
    ...
    def show_Name_listWidge(self):
        ...
        if self.sqlite_exist():
            ...
            print("There's no db_trans.sqlite file exists!")
    ...
    def init(self):
        ...
        if self.data_xls_exist():
            ...
            if re.findall(r'^windows.*', self.Platform, re.I):
                ...
                import win32api
                # 隐藏数据库文件
                win32api.SetFileAttributes('db_trans.sqlite',
