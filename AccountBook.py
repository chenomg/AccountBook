...
class AccountBook(QMainWindow):
    ...
    def __init__(self, db_file='db_file', title='通讯费'):
        ...
        self.ui.yearly_modify_pushButton.clicked.connect(
            self.yearly_modify_PushButtonClicked)
        # 如果有员工被选中则在右边显示相关信息
    ...
    def export_DB_months_selected(self):
        if self.sqlite_exist():
            text, ok = QInputDialog.getText(
                self, "Message", "请输入需要导出数据的月份\n月份:", QLineEdit.Normal,
                '1-{}'.format(self.get_month()[0]))
            if ok and text != '':
    ...
    def update_info(self):
        ...
        if infos:
            for info in infos:
                ...
                self.ui.yearly_lineEdit.setText(str(self.total_selected))
                self.ui.remain_lineEdit.setText(
                    str(round(self.remain_selected, 2)))
        else:
        ...
        if infos:
            history_text_list = []
            for info in infos:
                string = str(info[0]) + ":于" + str(
                    info[1]) + ', 提交:' + str(info[2]) + ', 共支取:' + str(
                        round(info[3], 2)) + ', 剩余:' + str(round(info[4], 2))
    ...
    def update_db(self, name, month=None, value=0, yearly_modify=None):
        # 更新数据库数据
        conn = sqlite3.connect(self.DB_FILE)
        cursor = conn.cursor()
        if not month:
        ...
        # 更新年度总额调整
        if yearly_modify:
            remain_value_update = yearly_modify - extracted_value_update
            cursor.execute(
                'UPDATE DATASHEET SET TOTAL = {total}, REMAIN = {remain} WHERE NAME = "{name}"'.
                format(
                    total=yearly_modify,
                    remain=remain_value_update,
                    name=str(name)))
        conn.commit()
        # 更新历史记录
        conn = sqlite3.connect(self.DB_FILE)
    ...
    def show_Selected_Info(self, item):
        ...
        self.ui.yearly_modify_pushButton.setEnabled(True)
    ...
    def show_Name_listWidge(self):
        ...
        if self.sqlite_exist():
            ...
            print("There's no {db_file} file exists!".format(self.DB_FILE))
    ...
    def yearly_modify_Value_Check(self, extracted_value, yearly_modify_value):
        # 用来检查年总额度调整是否有效（额度应大于目前已提取总额）
        if yearly_modify_value >= extracted_value:
            return True
        else:
            return False
    ...
    def cancel_PushButtonClicked(self):
        if self.cancel_count:
            msgBox = QMessageBox(
                ...
                '确定要撤销本次操作吗？\n' + self.cancel_name + ': ' + str(
                    self.cancel_value), QMessageBox.NoButton, self)
            msgBox.addButton("Yes!", QMessageBox.AcceptRole)
            msgBox.addButton("No", QMessageBox.RejectRole)
            if msgBox.exec_() == QMessageBox.AcceptRole:
                self.update_db(
    ...
    def yearly_modify_PushButtonClicked(self):
        # 变更年度总额
        if self.name_selected:
            ...
            if re.findall(r'^\d*\.?(\d|\d\d)?$', modify_str):
                ...
                if modify_value:
                    ...
                    conn = sqlite3.connect(self.DB_FILE)
                    cursor = conn.cursor()
                    sql = "SELECT EXTRACTED FROM DATASHEET WHERE NAME = '{name}'".format(
                        name=self.name_selected)
                    cursor.execute(sql)
                    infos = cursor.fetchall()
                    if infos:
                        extracted_value = infos[0][0]
                    else:
                        pass
                    if self.yearly_modify_Value_Check(extracted_value,
                                                      modify_value):
                        # 更新数据库数据
                        self.update_db(
                            name=self.name_selected,
                            yearly_modify=modify_value)
                        box = QMessageBox()
                        box.information(self, 'Message', '提交成功')
                        self.update_info()
                    else:
                        QMessageBox().information(self, 'Message', '额度调整无效.')
                    # 更新数据后将提交输入栏置零，防止误操作
                    self.ui.submit_lineEdit.setText('0')
                else:
                    box = QMessageBox()
    ...
    def init(self):
        ...
        if self.data_xls_exist():
            ...
            if re.findall(r'^windows.*', self.Platform, re.I):
                ...
                import win32api
                # 隐藏数据库文件
                win32api.SetFileAttributes(self.DB_FILE,
